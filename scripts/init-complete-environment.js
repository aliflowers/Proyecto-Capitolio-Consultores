...
/**
 * Script de inicialización completa del entorno de desarrollo
...
async function initAuditTable() {
  logStep(8, 'Inicializando tabla de auditoría...');
  
  try {
    logProgress('Creando tabla audit_logs si no existe...');
    // Ejecutar el script de inicialización de auditoría
    executeCommand('node scripts/init-audit-table.js', { stdio: 'pipe' });
    logSuccess('Tabla de auditoría inicializada correctamente');
    return true;
  } catch (error) {
    logError(`Error al inicializar tabla de auditoría: ${error.message}`);
    return false;
  }
}
...
async function runConnectionTests() {
  logStep(9, 'Ejecutando pruebas de conexión...');
...
async function showAccessInfo() {
  logStep(10, 'Información de acceso al entorno local');
...
async function main() {
  try {
    log('\n🚀 Iniciando configuración completa del entorno de desarrollo local para Nexus Jurídico...\n', colors.fg.cyan);
    ...
    if (!(await checkDockerDesktop())) {
      logWarning('Por favor, inicie Docker Desktop manualmente y vuelva a ejecutar este script');
      process.exit(1);
    }
    ...
    // Inicializar tabla de auditoría
    if (!(await initAuditTable())) {
      logWarning('No se pudo inicializar la tabla de auditoría, continuando...');
    }
    ...
    log('\n🎉 ¡Configuración completa del entorno de desarrollo local finalizada exitosamente!', colors.fg.green);
